...
class HyFrameData:
    ...
    def _ser_data(self):
        if isinstance(self.data, mathutils.Vector):
            if len(self.data) == 2:
                return serialize_vq(self.data, 2, 0)
            else:
                return serialize_vq(self.data, len(self.data), 4)
        elif isinstance(self.data, mathutils.Quaternion):
            return serialize_vq(self.data, 4, 5)
...
class HytaleSerializer:
    ...
    def serialize_scene(self, all_objects):
        armatures = self._collect_scene_armatures(all_objects)
        out = {}
        for anim in bpy.context.scene.sna_animations:
            out[anim.name] = self._base_animation(anim)
            self.data = {}
            ...
            for d in self.data:
                out[anim.name]["nodeAnimations"][d] = {
                    "position": [],
                    "orientation": [],
                    "shapeStretch": [],
                    "shapeVisible": [],
                ...
                self.data[d]["position"] = self._rdp(
                    ...
                    bpy.context.scene.sna_pos_exp_tresh,
                    self._vec_distance,
                )
                ...
                self.data[d]["orientation"] = self._rdp(
                    self.data[d]["orientation"],
                    bpy.context.scene.sna_rot_exp_tresh,
                    self._quat_distance,
                )

                for i in range(len(self.data[d]["position"])):
                    out[anim.name]["nodeAnimations"][d]["position"].append(
                        self.data[d]["position"][i].as_dict()
                    )
                for i in range(len(self.data[d]["orientation"])):
                    out[anim.name]["nodeAnimations"][d]["orientation"].append(
                        self.data[d]["orientation"][i].as_dict()
                    )

                empty = True
                for s in out[anim.name]["nodeAnimations"][d]:
                    if len(out[anim.name]["nodeAnimations"][d][s]) > 0:
                        empty = False
                if empty:
                    out[anim.name]["nodeAnimations"].pop(d)

        return out["test_anim"]
    ...
    def _rdp(self, points, epsilon, distance_func):
        ...
        #            next = points[i+1]
        #            curr = points[i]
        #            if next.get_rounded(4) < curr.get_rounded(4) and prev.get_rounded(4) > curr.get_rounded(4):
        #                pt.append(curr.frame)
        #            if next.get_rounded(4) > curr.get_rounded(4) and prev.get_rounded(4) < curr.get_rounded(4):
        #                pt.append(curr.frame)
        #
        #
    ...
    def _serialize_stretch(self, obj):
        if isinstance(obj, bpy.types.Bone):
            ...
            return serialize_vq(obj.scale.xzy, 3, 4)
